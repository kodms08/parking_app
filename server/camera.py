...
#def camera():
    ...
    while(True):
        ...
        if(ret) :
            ...
            if (cv2.waitKey(1) == ord('q')) or distance=="full":
            #if (cv2.waitKey(1) == ord('q')):
                cv2.imwrite('face.jpg',frame,params=[cv2.IMWRITE_PNG_COMPRESSION,0])
                cap.release()
                break
                

    
    img_ori = cv2.imread('face.jpg')
    ...
    for contour in contours:    ####### modify ########### to jisung
